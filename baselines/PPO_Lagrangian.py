...
from torch_utils.torch_utils import get_flat_params


save_dir = 'save-dir'


class PPO_Lag(Lagrangian):
    def __init__(
            self, state_dim, action_dim, capacity, batch_size, simulator, Monitor, max_kl, epochs, entropy_coef, epsilon,
            val_small_loss, cost_small_loss, discount_val, discount_cost, lambda_val, lambda_cost, ac_lr, val_lr,
            cost_limit,
            clip=0.2,
        ...
        # configs
        self.epochs = epochs
        self.episode_num = 15  # 0,2,5,15,19
    ...
    def update_Actor(self, states, actions, reward_advantage, cost_advantage):
        ...
        for i in range(len(states)):
            ...
            old_log_probs = torch.cat((old_log_probs, lg), 0)
            old_action_dists = torch.cat((old_action_dists, action_dist), 0)
        ...
        for j in range(self.epochs):
            ...
            for i in range(len(states)):
                ...
                dist_entropy = torch.distributions.Categorical(action_dist).entropy()  # .entropy().mean()
                action_dists_entropy = torch.cat((action_dists_entropy, torch.tensor([dist_entropy])), 0)

            ratio = torch.exp(log_probs - old_log_probs.detach())
            surr1 = ratio * reward_advantage
            surr2 = torch.clamp(ratio, 1 - self.clip, 1 + self.clip) * reward_advantage  # 截断
            ...
            mean_kl = mean_kl_first_fixed(old_action_dists.detach(), action_dists)  # KL散度的平均值
            # approx_kl = .5 * (log_probs - old_log_probs).mean().item()
            if mean_kl > 1.2 * self.max_kl:
    ...
    def save_session(self):
        ...
        save_path = os.path.join(save_dir, 'ppo_lag_model0406' + '.pt')
    ...
    def load_session(self):
        actor_load_path = os.path.join(save_dir, 'ppo_lag_model_0405_5%.pt')  # d=5%,效果：2>0>1;
...
if __name__ == '__main__':
    ...
    monitor = Monitor(train=True, spec="PPO_Lag_RL_Dispatch")
    ...
    model = PPO_Lag(state_dim=state_size, action_dim=action_size, simulator=environment, capacity=capacity,
    ...
    # for train
    n_episodes = 20  # train:565,test:20
    n_steps = 168  # 一天内
