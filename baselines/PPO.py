...
class PPO:
    def __init__(self, state_dim, action_dim, gamma, simulator, capacity_size, batch_size, actor_lr, critic_lr, epsilon,
                 lmbda, eps, epochs, Monitor, continue_from_file=False, save_every=1, print_updates=True):
        self.env = simulator
    ...
    def update(self, s, states, actions, rewards, next_s, t_actor_loss, t_critic_loss):
        ...
        advantage = compute_adv(self.gamma, self.lmbda, td_delta.cpu().detach().numpy())
    ...
    def save_session(self):
        ...
        save_path = os.path.join(save_dir, 'ppo_model0405' + '.pt')
    ...
    def load_session(self):
        actor_load_path = os.path.join(save_dir, 'ppo_model_0405' + '.pt')
...
if __name__ == '__main__':
    ...
    monitor = Monitor(train=True, spec="PPO_Dispatch")
    ...
    model = PPO(state_dim=state_size, action_dim=action_size, simulator=environment, actor_lr=1e-2, critic_lr=1e-2,
                eps=0.2, lmbda=0.98, capacity_size=capacity, batch_size=batch, Monitor=monitor, gamma=0.995,
                epsilon=0.05, epochs=10)
